        ...
        for (int i = 0; i < 1000; i++) {
            new Thread(() -> {
                ...
                for (int j = 0; j < route.length(); j++) {
                    if (route.charAt(j) == 'R') turnsNumber++;
                }
                synchronized (sizeToFreq) {
                    if (sizeToFreq.containsKey(turnsNumber)) {
                        sizeToFreq.put(turnsNumber, sizeToFreq.get(turnsNumber) + 1);
                    } else {
                        sizeToFreq.put(turnsNumber, 1);
                    }
                }
            }).start();
        }
